...
const regexRaporlar = /if \(\['Finansal Rapor', 'Faaliyet Raporu', 'Yatırımcı Sunumu', 'Araştırma Raporu', 'Özet Rapor Notları'\]\.includes\(activeTab\)\) \{[\s\S]*?\} else if \(activeTab === 'Hisse Notları'\)/;

const newRaporlarLogic = `if (activeTab === 'Raporlar') {
                const fsN = require('fs');
                const pathN = require('path');
                const reportsDir = pathN.join(process.cwd(), 'Hisseler', selectedHisse);
                let files = [];
                if (fsN.existsSync(reportsDir)) {
                    files = fsN.readdirSync(reportsDir).filter(f => f.toLowerCase().endsWith('.pdf') || f.toLowerCase().endsWith('.doc') || f.toLowerCase().endsWith('.docx'));
                }
                ...
                if (files.length === 0) {
                    ...
                    files.forEach(file => {
                        const filePath = 'Hisseler/' + selectedHisse + '/' + file;
                        contentHtml += \`<li style="margin-bottom: 0.5rem; padding: 0.5rem; background: rgba(255,255,255,0.05); border-radius: 4px;">
                            <a href="\${filePath}" target="_blank" style="color: var(--accent-color); text-decoration: none; font-weight: bold; display: flex; align-items: center; gap: 0.5rem;">
                ...
                let html = \`<div class="dash-card" style="display:flex; flex-direction:column; height:100%;"><div class="dash-title">Geleceğe İlişkin Beklentileriniz</div><div style="flex:1; overflow-x:auto;"><table class="dash-table compact-table" style="min-width: 1000px;"><thead>\${headerHtml}</thead><tbody>\`;
                ...
                rows.forEach(r => {
                    ...
                    years.forEach(y => {
                        ...
                        if (editMode && !r.readonly) {
                            html += \`<td><input type="number" step="any" style="width:100%; background:rgba(255,255,255,0.1); color:#fff; border:1px solid var(--accent-color); padding:4px; text-align:center; border-radius:4px;" value="\${val}" onchange="window.updateDegerlemeInput('\${selectedHisse}', '\${y}', '\${r.key}', parseFloat(this.value)||0)"></td>\`;
                        } else {
                            html += \`<td style="\${r.isTarget ? 'font-weight:bold; color:var(--success-color); font-size:1.1rem;' : ''}">\${displayVal}</td>\`;
                        }
                    });
                    html += \`</tr>\`;
                });
...
if (app.match(regexRaporlar)) {
    app = app.replace(regexRaporlar, newRaporlarLogic);
    fs.writeFileSync('e:\\Yunvest\\js\\app_v45.js', app, 'utf8');
    console.log('✅ Replaced Raporlar and added Değerleme');
} else {
    console.log('❌ Failed to find Raporlar logic block!');
}
