...
let code = fs.readFileSync('js/app_v49.js', 'utf8');
...
const target_html = '                        <div style="flex:1; min-height:400px; min-width: 0; position:relative;"><canvas id="chart-combined-brut"></canvas></div>\n                    </div>\n                </div>\n                ;';
if (code.includes(target_html)) {
...
const target_js = "                const ctxFavok = document.getElementById('chart-ceyreklik-favok');";
if (code.includes(target_js)) {
    code = code.replace(target_js, js_to_add + "\n" + target_js);
} else {
    console.log("Failed to inject JS");
}

fs.writeFileSync('js/app_v49.js', code, 'utf8');
console.log("Done");
